...
from langkit.dsl import ASTNode, Field, abstract, EnumNode, T
...
class SingleParam(PythonNode):
    is_varargs = Field(type=T.BoolType)
    is_kwargs = Field(type=T.BoolType)
...
class Id(Name):
    id = Field(type=T.TokenType)

    sym = Property(
        Self.id.symbol, doc="Shortcut to get the symbol of this node"
    )


class NumberLit(Expr):
    num = Field(type=T.TokenType)


class StringLit(Expr):
    str = Field(type=T.TokenType)
...
python_grammar.add_rules(
    name=Id(L.Identifier(keep=True)),
